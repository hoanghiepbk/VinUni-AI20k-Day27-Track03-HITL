    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        a: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": (
                ...
                "Calibrate self-reported confidence aggressively:\n"
                "  - 0.80-0.95: trivial mechanical changes.\n"
                "  - 0.60-0.79: small features with one or two open questions.\n"
                "  - 0.40-0.59: feature touches auth/crypto/migrations OR has "
                "2+ correctness issues. Always populate risk_factors.\n"
                "  - 0.10-0.39: multiple red flags (weak hashing, plaintext "
                "secrets, SQL injection, missing tests, hard-coded creds). "
                "Populate escalation_questions with 2-4 file-anchored "
...
async def node_route(state):
    ...
    decision_map = {"auto_approve": "auto", "escalate": "escalate", "human_approval": "pending"}
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="route",
        confidence=c,
        ...
        reason=f"routed to {decision} based on confidence {c:.2%}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    if state.get("escalation_answers") or state.get("human_choice") == "approve":
        ...
        action = "rejected"
    a = state["analysis"]
    is_hitl = state.get("human_choice") is not None or state.get("escalation_answers") is not None
    commit_decision = "approve" if action == "committed" else (
        "reject" if action == "rejected" else "pending"
    )
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="commit",
        confidence=a.confidence,
        risk_level=risk_level_for(a.confidence),
        reviewer_id=os.environ.get("GITHUB_USER") if is_hitl else None,
        decision=commit_decision,
        reason=f"commit result: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"auto-posted high-confidence review ({action})",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason="asking reviewer: " + " | ".join(questions),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    t1 = time.monotonic()
    answers_summary = "; ".join(f"{q} -> {ans}" for q, ans in (answers or {}).items())
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=f"reviewer answers: {answers_summary}",
        execution_time_ms=int((time.monotonic() - t1) * 1000),
    ))
