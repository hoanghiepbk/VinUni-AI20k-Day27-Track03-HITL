    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If confidence is below 0.60, you MUST populate "
                "escalation_questions with 2-4 file-anchored questions "
                "(e.g. 'Why MD5 in auth.py:hash_password?')."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": (
                "You are a senior code reviewer producing a REFINED review "
                "after receiving clarifying answers from a human reviewer. "
                "Incorporate the answers into your reasoning, raise confidence "
                "if the answers resolve your concerns, and produce concrete "
                "review comments. Structured output."
            )},
            {"role": "user", "content": (
                f"Original diff:\n{state['pr_diff']}\n\n"
                ...
                f"Reviewer answers to clarifying questions:\n{qa_block}"
            )},
        ])
