...
async def _recent_threads() -> list[dict]:
...
with st.sidebar:
    ...
    try:
        threads = asyncio.run(_recent_threads())
    except Exception as e:  # DB may not exist yet on first run
        threads = []
        st.caption(f"(no sessions yet — {e})")
    for t in threads:
        label = (
            f"{t['pr_url'].rsplit('/', 1)[-1] or t['pr_url']}  "
            f"·  risk={t['worst_risk']}  ·  {t['events']} evts"
        )
        if st.button(label, key=f"sess_{t['thread_id']}"):
            st.session_state.thread_id = t["thread_id"]
            st.session_state.pr_url = t["pr_url"]
            st.session_state.interrupt_payload = None
            st.session_state.final = None
            st.rerun()
        st.caption(f"`{t['thread_id'][:8]}…`  {t['last_event']}")
